"""
Spectral broadening functions for synthetic spectra.
Translated from Smooth.f and Vmacro.f.

Entry point: smooth() — pure numpy, no State dependency.
"""
import numpy as np


# ---------------------------------------------------------------------------
# Radial-tangential macroturbulence profile  (Vmacro.f)
# ---------------------------------------------------------------------------
# Gray (1992), normalized so integral = 1.  250 tabulated points.
...
def _vmacro(x: float) -> float:
    """Linear interpolation into the RT macroturbulence profile table."""
    if x < _XRT[0] or x >= _XRT[-1]:
        return 0.0
    i = int(round(100.0 * x))   # Fortran: nint(100.*xprofile)
    return (_YRT[i + 1] - _YRT[i]) * (x - _XRT[i]) / (_XRT[i + 1] - _XRT[i]) + _YRT[i]


# ---------------------------------------------------------------------------
# Symmetric kernel convolution helper
# ---------------------------------------------------------------------------

def _apply_kernel(y: np.ndarray, center: float, wings: np.ndarray,
                  norm: float) -> np.ndarray:
    """
    Convolve y with a symmetric kernel whose value at offset 0 is `center`
    and at offset ±j (j=1..len(wings)) is wings[j-1].

    Edge points (within len(wings) of each end) are set to 1.0 (continuum).

    Matches the Fortran convention in Smooth.f exactly:
      z(i) = center*y(i) + sum_j wings(j)*(y(i-j)+y(i+j))
      z(i) /= norm
    with z(i) = 1 for edge points.
    """
    n = len(y)
    hw = len(wings)          # half-width: jdelrot / jdelmac / jdel

    # Build full kernel: [wings[-1], ..., wings[0], center, wings[0], ..., wings[-1]]
    kernel = np.empty(2 * hw + 1)
    kernel[hw] = center
    kernel[hw + 1:] = wings
    kernel[:hw] = wings[::-1]

    # np.convolve 'valid' mode produces exactly n - 2*hw interior points
    interior = np.convolve(y, kernel, mode='valid') / norm

    z = np.ones(n, dtype=float)    # edges default to 1.0 (continuum)
    z[hw:n - hw] = interior
    return z
...
def smooth(wave_arr, flux_arr, step: float,
           vsini: float = 0.0,
           limbdark: float = 0.0,
           vmac: float = 0.0,
           fwhmgauss: float = 0.0,
           fwhmloren: float = 0.0,
           addflux: float = 0.0) -> np.ndarray:
    """
    Apply spectral broadening to a normalised flux array.

    Parameters
    ----------
    wave_arr  : wavelength grid [Å]
    flux_arr  : normalised flux (continuum = 1)
    step      : uniform wavelength step [Å]
    vsini     : projected rotation [km/s]; 0 = skip
    limbdark  : linear limb darkening coefficient (used with vsini)
    vmac      : radial-tangential macroturbulence [km/s]; 0 = skip
    fwhmgauss : Gaussian FWHM [Å]; 0 = skip
    fwhmloren : Lorentzian FWHM [Å]; 0 = skip (mutually exclusive with Gaussian)
    addflux   : veiling fraction (extra continuum); 0 = skip

    Returns
    -------
    np.ndarray of the same length as flux_arr
    """
    y = np.array(flux_arr, dtype=float)
    kount = len(y)
    wave_cen = (wave_arr[0] + wave_arr[-1]) / 2.0

    # --- veiling ---
    if addflux > 0.0:
        y = (y + addflux) / (1.0 + addflux)

    # --- rotation broadening (Gray 1976) ---
    if vsini > 0.0:
        dlamlim = wave_cen * vsini / 3.0e5
        if step < dlamlim:
            pi = np.pi
            bottom = dlamlim * pi * (1.0 - limbdark / 3.0)
            c1 = 2.0 * (1.0 - limbdark) / bottom
            c2 = 0.5 * limbdark * pi / bottom
            prot0 = c1 + c2
            jdelrot = int(dlamlim / step)
            if jdelrot > 1000:
                raise ValueError(f"Rotational profile too large (vsini={vsini} km/s, "
                                 f"jdelrot={jdelrot} > 1000)")
            if jdelrot > kount // 4:
                raise ValueError(f"Rotational profile covers whole spectrum "
                                 f"(jdelrot={jdelrot} > kount/4={kount//4})")
            # prot_arr[j-1] = profile at dlam=(j-1)*step (j=1..jdelrot, Fortran indexing)
            dlam = np.arange(jdelrot, dtype=float) * step   # (i-1)*step for i=1..jdelrot
            term = 1.0 - (dlam / dlamlim) ** 2
            prot_arr = c1 * np.sqrt(term) + c2 * term
            powerrot = prot0 + 2.0 * prot_arr.sum()
            y = _apply_kernel(y, prot0, prot_arr, powerrot)

    # --- macroturbulence broadening (radial-tangential profile) ---
    if vmac > 0.0:
        wavemac = wave_cen * vmac / 3.0e5
        pmac = []
        for i in range(1, 1001):
            ...
            raise ValueError(f"Macroturbulent profile too large (vmac={vmac} km/s, "
                             f"wavemac={wavemac:.3f} Å)")
        if jdelmac > kount // 4:
            raise ValueError(f"Macroturbulent profile covers whole spectrum")
        pmac_arr = np.array(pmac[:jdelmac])
        powermac = 1.0 + 2.0 * pmac_arr.sum()
        y = _apply_kernel(y, 1.0, pmac_arr, powermac)

    # --- Gaussian or Lorentzian broadening (mutually exclusive) ---
    if fwhmgauss > 0.0 or fwhmloren > 0.0:
        use_gauss = fwhmgauss > 0.0
        if use_gauss:
            sigma = fwhmgauss / 2.0
            aa = 0.6932 / sigma ** 2      # ln(2) / sigma^2 → correct FWHM
        else:
            sigma = fwhmloren / 2.0
        ...
        for i in range(1, 1001):
            x = step * i
            val = (np.exp(-aa * x ** 2) if use_gauss
                   else sigma ** 2 / (sigma ** 2 + x ** 2))
            p.append(val)
            if val < 0.02:
                jdel = i
                break
        else:
            raise ValueError(f"Gaussian/Lorentzian profile too large "
                             f"(sigma={sigma:.3f} Å)")
        if jdel > kount // 4:
            raise ValueError(f"Gaussian/Lorentzian profile covers whole spectrum")
        p_arr = np.array(p[:jdel])
        power = 1.0 + 2.0 * p_arr.sum()
        y = _apply_kernel(y, 1.0, p_arr, power)
